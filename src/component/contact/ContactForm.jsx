...
// import { MapPin, Phone, Mail, Send, Facebook, Globe, Instagram, Linkedin } from "lucide-react";
...
//         style={{ 
//           // Using a placeholder agro/nature image. Replace with your specific image path.
//           backgroundImage: "url('/assets/vision/sunny_day.jpg')" 
//         }}
//       ></div>
...
// Component
// ==============================
const ContactForm = ({ backgroundImage = '/assets/vision/sunny_day.jpg' }) => {
  const [formData, setFormData] = useState({
    name: "",
    email: "",
    message: ""
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Start animations after image loads
      setTimeout(() => {
    ...
    // Fallback: start animations after timeout if image takes too long
  ...
  const backgroundStyle = {
    ...
    backgroundColor: isImageLoaded ? 'transparent' : '#0f172a', // slate-900 fallback
  };

  return (
    <section 
      ref={sectionRef}
      className="relative py-24 lg:py-32 font-sans overflow-hidden bg-slate-900"
    >
      
      {/* 1. Background Image with Parallax-like feel */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-fixed bg-no-repeat z-0 transition-opacity duration-1000"
        style={backgroundStyle}
      >
        {/* Loading overlay */}
        <div className={`absolute inset-0 bg-slate-900 transition-opacity duration-700 ${isImageLoaded ? 'opacity-0' : 'opacity-100'}`}></div>
      </div>
